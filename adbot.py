...
DEFAULT_CONFIG = {
    "host_usernames": ["800.98"],
    "admin_usernames": ["800.98"],
    "vip_usernames": [],
    "banned_users": [],
    "teleport_locations": {
    ...
    "language": "fa",
    "welcome_message": "<#FFD700>✨🌟𝐖𝐞𝐥𝐜𝐨𝐦𝐞 {username} ❤️ 𝐆𝐥𝐚𝐝 𝐭𝐨 𝐡𝐚𝐯𝐞 𝐲𝐨𝐮 𝐡𝐞𝐫𝐞!\n🕺 𝐔𝐬𝐞 𝐍𝐮𝐦𝐛𝐞𝐫𝐬 (𝟏-𝟐𝟒𝟖) 𝐨𝐫 𝐄𝐦𝐨𝐭𝐞 𝐍𝐚𝐦𝐞𝐬 𝐭𝐨 𝐝𝐚𝐧𝐜𝐞!\n👑 𝐓𝐡𝐢𝐬 𝐛𝐨𝐭 𝐰𝐚𝐬 𝐜𝐫𝐞𝐚𝐭𝐞𝐝 & 𝐝𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐝 𝐛𝐲 @800.98 😉\n📺 𝐒𝐮𝐛𝐬𝐜𝐫𝐢𝐛𝐞 𝐭𝐨 𝐨𝐮𝐫 𝐘𝐨𝐮𝐓𝐮𝐛𝐞: @𝐚𝐝𝐨𝐫𝐢_𝐡𝐢𝐭𝐨𝐭𝐬𝐨 🚀",
    "announcement_interval": 300,
    "announcement_message": "<#0000FF>برای اجاره بات به آیدی 800.98@ پیام دهید!"
}
    ...
    def get_message(self, key, **kwargs):
        messages = {
            "fa": {
                ...
                "invalid_command": "<#FF0000>❌دستور نامعلوم! برای دیدن دستورات بات !help استفاده کنید یا به @800.98 پیام بدید.",
                "no_permission": "<#0066FF>فقط ادمین‌ها می‌توانند از این دستور استفاده کنند!",
                "user_not_found": "<#7CFF6B>کاربر {username} آنلاین نیست.",
                "invalid_format": "<#FFD700>فرمت نادرست: {format}",
                "teleport_success": "<#0000FF>@{username} به {location} تلپورت شد!",
                "teleport_error": "<#FF0000>خطا در تلپورت: {error}",
                "heart_success": "<#0066FF>{count} قلب بنفش به @{username} ارسال شد!",
                "heart_all_success": "<#FF0000>{count} واکنش به {count} نفر ارسال شد!",
                "clap_success": "<#FFD700>{count} clap به @{username} ارسال شد!",
                "wink_success": "<#0000FF>{count} wink به @{username} ارسال شد!",
                "wave_success": "<#FF0000>{count} wave به @{username} ارسال شد!",
                "thumbs_success": "<#7CFF6B>{count} thumbs-up به @{username} ارسال شد!",
                "wallet_error": "<#0066FF>خطا در دریافت موجودی: {error}",
                "tip_success": "<#FFD700>{amount} گلد به @{username} ارسال شد.",
                "tip_all_success": "<#7CFF6B>تیپ {amount} گلد به {count} نفر ارسال شد!",
                "ban_success": "<#0066FF>@{username} بن شد!",
                "unban_success": "<#7CFF6B>کاربر @{username} با موفقیت آنبن شد!",
                "unban_not_banned": "<#FF0000>کاربر @{username} در لیست بن نیست.",
                "dancechain_success": "<#FF0000>زنجیره رقص برای @{username} اجرا شد!",
                "addtele_success": "<#FF0000>مکان {location} ذخیره شد!",
                "deltele_success": "<#7CFF6B>مکان {location} با موفقیت حذف شد!",
                "deltele_not_found": "<#0066FF>مکان {location} وجود ندارد!",
                "deltele_protected": "<#FFD700>نمی‌توانید مکان پیش‌فرض {location} را حذف کنید!",
                "set_item_success": "<#0000FF>ظاهر ربات به ایتم‌های @{username} تغییر کرد!",
                "listadd_empty": "<#FF0000>هیچ ادمینی در لیست وجود ندارد.",
                "listadd_success": "<#0066FF>لیست ادمین‌ها ({count} نفر):\n{admin_list}",
                "freeze_success": "<#7CFF6B>کاربر @{username} فریز شد!",
                "unfreeze_success": "<#FFD700>کاربر @{username} از حالت فریز آزاد شد!",
                "unfreeze_not_frozen": "<#0000FF>کاربر @{username} فریز نشده است!",
                "party_success": "<#FF0000>رقص شماره {dance_number} برای @{username} فعال شد!",
                "party_all_success": "<#FF0000>رقص شماره {dance_number} برای {count} کاربر فعال شد!",
                "partys_success": "<#FFD700>رقص اجباری برای @{username} متوقف شد!",
                "partys_not_dancing": "<#7CFF6B>کاربر @{username} در حال رقص اجباری نیست!"
            }
        }
    ...
    async def on_chat(self, user: User, message: str):
        ...
        try:
            ...
            if msg_lower in self.emotes:
                ...
                await self.highrise.chat("<#7CFF6B>سازنده این بات:@800.98 👑")
